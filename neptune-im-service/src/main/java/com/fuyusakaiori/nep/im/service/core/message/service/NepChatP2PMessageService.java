...
public class NepChatP2PMessageService {
    ...
    public void handleMessage(NepChatP2PMessage message){
        ...
        // 3. 校验消息是否可以发送: 校验双方是否为好友, 校验双方彼此是否在黑名单中
        if (applicationConfig.isSendMessageCheckFriendship()){
            // 3.1 校验好友关系的状态: 0 表示双方都是好友, 1 表示双方彼此不是好友, 2 表示左侧不是右侧的好友, 3 表示右侧不是左侧的好友
        ...
        if (applicationConfig.isSendMessageCheckFriendshipBlack()){
            ...
            // 3.4 根据拉黑关系决定是否继续: 先简单化, 只要有一方拉黑就不允许发送消息
        ...
        // TODO 3. 响应 ACK 消息给客户端

        // TODO 4. 同步消息给自己的所有客户端

        // 5. 发送消息给对方的所有客户端
        messageProducer.sendMessage(appId, message.getMessageType(), message.getToUserId(), message);
    }
